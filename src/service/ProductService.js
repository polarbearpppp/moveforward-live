...
export const PointService = {
        ...
        return [
            {
                ...
                partyscore: [
                    ...
                    },
                    {
                      ...
                      name: 'ไทยสร้างไทย',
                      score: Math.floor(Math.random() * 101),
                      scorepercen: Math.floor(Math.random() * 101)
                      },
                      {
                          image: 'test1',
                          name: 'เสรีรวมไทย',
    ...
    getTotalScore() {
      ...
      let totalScore = {};
      let scoreAll = 0;
      for (const key of dataAll) {
        console.log(`Key: ${key.name}`);
        for (const party of key.partyscore){
          console.log(`party name: ${party.name}`)
          console.log(`party: ${party.score}`)
          if ( (party.name) in totalScore){
            totalScore[party.name] += party.score
          }else{
            totalScore[party.name] = party.score;
          }
          scoreAll += party.score
        }
      }
      for (const key of Object.keys(totalScore)) {
        const value = totalScore[key];
        console.log(`Key: ${key}, Value: ${value}`);
      }
    return  Promise.resolve([ {
      image: 'test1',
      name: 'ก้าวไกล',
      score: totalScore['ก้าวไกล'] ,
      scorepercen: (totalScore['ก้าวไกล'] / scoreAll )*100
      },
      {
          image: 'test1',
          name: 'ประชาธิปัตย์',
          score: totalScore['ประชาธิปัตย์'],
          scorepercen: (totalScore['ประชาธิปัตย์'] / scoreAll )*100
      },
      {
          image: 'test1',
          name: 'เพื่อไทย',
          score: totalScore['เพื่อไทย'],
          scorepercen: (totalScore['เพื่อไทย'] / scoreAll )*100
      },
      {
        image: 'test1',
        name: 'ไทยสร้างไทย',
        score: totalScore['ไทยสร้างไทย'],
        scorepercen: (totalScore['ไทยสร้างไทย'] / scoreAll )*100
        },
        {
            image: 'test1',
            name: 'เสรีรวมไทย',
            score: totalScore['เสรีรวมไทย'],
            scorepercen: (totalScore['เสรีรวมไทย'] / scoreAll )*100
        },
        {
            image: 'test1',
            name: 'พลังประชารัฐ',
            score: totalScore['พลังประชารัฐ'],
            scorepercen: (totalScore['พลังประชารัฐ'] / scoreAll )*100
        },
        {
          image: 'test1',
          name: 'รวมไทยสร้างชาติ',
          score: totalScore['รวมไทยสร้างชาติ'],
          scorepercen: (totalScore['รวมไทยสร้างชาติ'] / scoreAll )*100
      },
      {
        image: 'test1',
        name: 'ภูมิใจไทย',
        score: totalScore['ภูมิใจไทย'],
        scorepercen: (totalScore['ภูมิใจไทย'] / scoreAll )*100
    },
    {
      image: 'test1',
      name: 'อื่นๆ',
      score: totalScore['อื่นๆ'],
      scorepercen:(totalScore['อื่นๆ'] / scoreAll )*100
  },
    ])
    },
      ...
      return [
        {
          ...
          price: 65,
          activity: Math.floor(Math.random() * 101),
          status: 'unqualified',
